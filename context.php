    ...
    switch ($type) {
        ...
        case 'login':
            ...
            if (count($user) && $accountExpired != 'accountExpired') {
                ...
                $_SESSION['isAdmin'] = $user->email == 'info@clientrdtmarkets.com';
        ...
        case 'getRegister':
            if ($_POST) 
            {
                ...
                elseif(filter_var($clientEmail, FILTER_VALIDATE_EMAIL))
                {
                    ...
                    $adminEmail = 'info@clientrdtmarkets.com';
                    ...
                    try 
                    {
                        ...
                        $mail->Mailer = 'smtp';
                        //Recipients
                        $mail->setFrom(EMAIL_FROM, 'Rdtmarkets');   // Name is optional
                    ...
                    if ($result == true) 
                    {
                        $subject = 'Your Rdtmarkets login details';
        ...
        case 'forgotPassword': 
            if ($_POST['type'] == 'request') {
                ...
                try {
                    ...
                    $mail->Port       = 587;                                    // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above
                    //Recipients
                    $mail->setFrom('info@clientrdtmarkets.com', 'Rdtmarkets');   // Name is optional
                    $mail->addAddress($clientEmail);
                    // Content
                    //$mail->SMTPDebug  = 1;
                    $mail->isHTML(true);                                  // Set email format to HTML
        ...
        case 'messages':
            ...
            foreach ($result as $key => $row) {
                if($row->email == 'info@clientrdtmarkets.com'){
                    $user['id'] = $row->id;
        ...
        case 'agentMessages':
            ...
            $adminUser = dbAction('getUserByEmail', (object) array('email' => 'info@clientrdtmarkets.com'));
...
function dbAction($type, $params)
{
    ...
    switch ($type) {
        ...
        case 'matcholdpassword':
            $sql = "SELECT * from users where password = '".$params->oldpassword."' and email = 'info@clientrdtmarkets.com'";
        break;

        case 'updatenewpassword':
            $sql = "UPDATE users SET password = '".$params->newPassword."' where email = 'info@clientrdtmarkets.com'";
        break;
        ...
        case 'getAdminId':
            $sql = "SELECT id from users where email = 'info@clientrdtmarkets.com'";
        break;
        ...
        case 'getUsers':
            $sql = "SELECT * FROM users where email <> 'info@clientrdtmarkets.com' and active = 1 ORDER BY id asc";
        break;
        
        case 'getUsersAndLastTradesNum':
            $sql = "SELECT trades.tradeNumber as last_trades_num, users.* FROM users left join trades on users.id = trades.`userId` where trades.id in (select max(id) as last_id from trades group by userId) and users.`email` <> 'info@clientrdtmarkets.com' and users.`active` = 1";
        break;
        ...
        case 'getUsersbyPage':
            $sql = "SELECT * FROM users where email <> 'info@clientrdtmarkets.com' and active = 1 ORDER BY id asc limit ".$params->page_first_result.",".$params->results_per_page;
        break;

        case 'getAgentUsersbyPage':
            $sql = "SELECT * FROM users where email <> 'info@clientrdtmarkets.com' and active = 1 and agent_role = 1 ORDER BY id asc";
        break;
        ...
        case 'getClientsOfNormalUser':
            $sql = "SELECT * FROM users WHERE  (active =1 AND email='info@clientrdtmarkets.com' ) OR (id = '".$params->id."' AND active = 1);";    
        break;
        ...
        case 'getAgentUserById':
            $sql = "SELECT * FROM users where email <> 'info@clientrdtmarkets.com' and active = 1 and agent_role = 1 and id = '".$params->user_id."' ORDER BY id asc";
        break;

        case 'getUsersOfAgentAccountbyPage':
            $sql = "SELECT * FROM users where email <> 'info@clientrdtmarkets.com' and active = 1 and agent_role = 0 and agent_id = ".$params->user_id." ORDER BY id asc";
            break;
        ...
        case 'getAvaiableClient':
            $sql = "SELECT * FROM cointrad_coin.`users` WHERE email <> 'info@clientrdtmarkets.com' AND active = 1 AND agent_id = 0 AND agent_role = 0";
            break;
